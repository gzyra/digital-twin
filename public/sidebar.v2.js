...
function renderMarkdown(text) {
  ...
  // First, convert escaped newlines (\n as text) to actual newlines for processing
  normalized = normalized.replace(/\\n/g, '\n');
  // Recover common one-line markdown emitted by LLMs.
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Group a contiguous run of pipe rows and render it as a real HTML table.
    if (line.startsWith('|')) {
      const tbl = [];
      while (i < lines.length && lines[i].startsWith('|')) { tbl.push(lines[i]); i++; }
      i--; // step back; the for-loop will advance
      if (tbl.length >= 3) { out.push(renderMarkdownTable(tbl.join('\n'))); }
      else { tbl.forEach(t => out.push('<div class="db-line">' + esc(t) + '</div>')); }
      continue;
    }

    if (line.startsWith('### ')) {
      out.push('<h5>' + esc(line.slice(4)) + '</h5>');
      continue;
    }
    if (line.startsWith('## ')) {
    ...
    if (kv) {
      out.push('<div class="db-line"><strong>' + esc(kv[1]) + ':</strong> ' + esc(kv[2] || '') + '</div>');
      continue;
    }

    const boldLine = esc(line).replace(/\*\*([^*]+)\*\*/g, '<strong>$1</strong>');
    out.push('<div class="db-line">' + boldLine + '</div>');
  }
  ...
  panel.innerHTML = entries.map(function(entry) {
    ...
    if (d.status === 'loading') {
      ...
      body = Object.entries(d.outputs).map(function(kv) {
        ...
        if (typeof val === 'string' && key.endsWith('_formatted')) {
          return '<div class="db-output-label">' + esc(key.replace(/_/g, ' ')) + '</div><div class="db-markdown">' + renderMarkdown(val) + '</div>';
        }
        if (val !== null && typeof val === 'object') {
          return '<div class="db-output-label">' + esc(key.replace(/_/g, ' ')) + '</div><pre class="db-raw">' + esc(JSON.stringify(val, null, 2)) + '</pre>';
        }
        if (isMarkdownTable(val))
          return '<div class="db-output-label">' + esc(key.replace(/_/g, ' ')) + '</div>' + renderMarkdownTable(val);
        // Check if value contains markdown formatting (##, **, \n, or literal \n) OR key=value pattern (semicolon-separated)
        const valStr = String(val);
        const isMarkdown = typeof val === 'string' && (valStr.includes('##') || valStr.includes('**') || valStr.includes('\n') || valStr.includes('\\n') || (valStr.includes('=') && valStr.includes(';')) || valStr.includes(':'));
        if (isMarkdown) {
          return '<div class="db-output-label">' + esc(key.replace(/_/g, ' ')) + '</div><div class="db-markdown">' + renderMarkdown(val) + '</div>';
        }
